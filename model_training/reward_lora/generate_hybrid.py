...
def add_lora_to_pipeline(pipe, model_manager, lora_path=None, lora_alpha=1.0,
    ...
    推理时的正确顺序：
      1. 先加 SFT LoRA（融合进 base，作为 DPO LoRA 的起点）
      2. 再加 DPO 训练得到的新 LoRA

    这与训练时的结构完全一致：
      训练时 base = base DiT + SFT LoRA fused
      训练时 policy = fused base + new DPO LoRA
    """
    if sft_lora_path:
...
def main(cfg: DictConfig):
    ...
    if cfg.get('lora_path', None) is not None and cfg.get('run_lora', True):
        ...
        # 如果有已有的 baseline，只对有对应 baseline 的样本生成 LoRA 视频
