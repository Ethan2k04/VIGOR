...
# ---------------------------------------------------------------------------
# Inject the vggt submodule (third_party/vggt) so `import vggt` resolves.
# ---------------------------------------------------------------------------
import sys

_EVALUATOR_DIR = os.path.dirname(os.path.abspath(__file__))     # rewards/evaluator
_REPO_ROOT = os.path.dirname(os.path.dirname(_EVALUATOR_DIR))   # VIGOR repo root
_VGGT_DIR = os.path.join(_REPO_ROOT, "third_party", "vggt")
if _VGGT_DIR not in sys.path:
    sys.path.insert(0, _VGGT_DIR)
